...
namespace pyjson
{
    ...
    inline nl::json to_json(const py::handle& obj)
    {
        ...
        if (py::isinstance<py::dict>(obj))
        {
            ...
            //// SPECIALIZED CODE TO GGSOLVER -------------------------------------------------------------------
            ////  1. Check if dictionary has exactly two keys (__ggsolver_header and __ggsolver_attr)
            //bool hasHeader = false, hasAttr = false, hasAnythingElse = false;
            //for (const py::handle& key : obj) {
            //    auto name = py::str(key).cast<std::string>();
            //    if (name == std::string("__ggsolver_header")) {
            //        hasHeader = true;
            //    }
            //    else if (name == std::string("__ggsolver_attr")) {
            //        hasAttr = true;
            //    }
            //    else{
            ...
            //}
            //
            ////  2. If dictionary is indeed an Entity, construct one.
            //if (hasHeader && hasAttr & !hasAnythingElse) {
            //    json attrMap;
            //    attrMap["__ggsolver_header"] = to_json(obj["__ggsolver_header"]);
            //    attrMap["__ggsolver_attr"] = to_json(obj["__ggsolver_attr"]);
            //    TEntity ent(attrMap);
            //    out = ent;
            //    return out;
            //}
